...
urlpatterns = [
    ...
    path(
        "add-account/", AuthenticateAccountView.as_view(), name="authenticate_account"
    ),
    path("delete/<int:account_id>/", DeleteAccount.as_view(), name="delete_account"),
    path(
        "trade-accounts/",
        TradeAccountViewSet.as_view({"get": "list", "post": "create"}),
        name="trade-account-list",
    ),
    path(
        "trade-accounts/<int:pk>/",
        TradeAccountViewSet.as_view(
            {
                "get": "retrieve",
                "put": "update",
    ...
    path(
        "statistics/",
        ComprehensiveTradeStatisticsView.as_view(),
        name="comprehensive-trade-statistics",
    ),
    path(
        "account-balance/",
        AccountBalanceView.as_view(),
        name="account-balance-statistics",
    ),
    path(
        "account-performance/",
        AccountPerformanceView.as_view(),
        name="trade-account-performance",
    ),
    # Explicitly add trade notes URLs
    path(
        "trade-notes/",
        TradeNoteViewSet.as_view({"get": "list", "post": "create"}),
        name="tradenote-list",
    ),
    path(
        "trade-notes/<int:pk>/",
        TradeNoteViewSet.as_view(
            {
                "get": "retrieve",
                "put": "update",
                "patch": "partial_update",
    ...
    path(
        "get_all_accounts/",
        AccountsSummaryView.as_view(),
        name="get-all-trade-accounts",
    ),
    ...
    path(
        "toggle-account-mode/<str:account_id>/",
        ToggleUserAccountStatus.as_view(),
        name="toggle-account-mode",
    ),
]
