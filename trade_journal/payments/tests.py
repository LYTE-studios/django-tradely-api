...
class TestGetDynamicEmailBackend(unittest.TestCase):
    ...
    def test_mailjet_backend(self, mock_import_module):
        ...
        mock_backend_class.EmailBackend.assert_called_once_with(
    ...
    def test_unsupported_service(self):
        ...
        self.assertEqual(
            str(context.exception), "Unsupported email service: UnsupportedService"
        )
...
class SendEmailSerializerTest(APITestCase):
    def test_send_email_serializer(self):
        ...
        self.assertEqual(
            serializer.validated_data["recipient_list"], data["recipient_list"]
        )
        ...
        self.assertEqual(
            serializer.validated_data["email_service_name"], data["email_service_name"]
        )
        self.assertEqual(
            serializer.validated_data["email_service_api_key"],
            data["email_service_api_key"],
        )
        self.assertEqual(
            serializer.validated_data["email_service_api_secret"],
            data["email_service_api_secret"],
        )
